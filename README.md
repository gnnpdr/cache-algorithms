...
```
cmake -DCMAKE_BUILD_TYPE=Release -B build -S . && cmake --build build -j4
```
...
./build/Belady
```
запустят соответствующий кэш. Заполнение с командной строки после запуска в порядке: размер кэша, количество элементов и сами элементы. Будет выведено количество попаданий. Кэши шаблонные.

можно перейти в папку с тестами и собрать там:
```
cd tests
cmake -DCMAKE_BUILD_TYPE=Release -B build -S . && cmake --build build -j4
```
...
./build/Belady_tests
```
запустят соответствующие программные тесты и выведут результат. Содержание тестов можно увидетm в соответствующих файлах tests/include/cachename_tests.hpp.

```
./build/efficiency_test
```
измерит эффективность работы разных алгоритмов кэширования и выведет процент попаданий при измерении на одном большом тесте в 400000 запросов. Содержание теста в файле tests/include/big_test.dat.
